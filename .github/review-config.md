...
- The `panenco/claude-review@v3` self-references (one in the caller workflow, one inside `pr-review.yml`'s install step) are intentional moving-tag references and are listed under `bugbot.md` → "Accepted supply-chain trade-offs". Do not flag.
