...
- **Pipeline-ref alignment is required for non-`@v1` consumers.** GitHub does not expose the called workflow's resolved ref to the called workflow itself, so `pr-review.yml` accepts `pipeline_ref` (default `'v1'`) and the install step checks out `panenco/claude-review` at that ref. Consumers pinning the workflow file at a non-default ref (a SHA, a feature branch) MUST also pass `with: pipeline_ref: <same-ref>`; otherwise the workflow runs new orchestration with old `@v1` skills and the build-context agent hits max-turns. Preserve this note when editing tag/release guidance.
...
- The caller workflow `.github/workflows/claude-review.yml` calls the reusable workflow via the **local path** `./.github/workflows/pr-review.yml`, not the cross-repo form `panenco/claude-review/.github/workflows/pr-review.yml@v1` that consumers use. Reason: the `org/repo/...@tag` form tripped `startup_failure` on this repo even with explicit secret mappings (documented quirk in `prompts/setup-review.md` Step 2). The local path also exercises in-PR pipeline changes against the PR's own review — stronger dogfood. Do not flag this as "diverging from the documented Quick Start" — the divergence is intentional and documented in the workflow's inline comment.
- The reusable workflow's downstream install step checks out `panenco/claude-review` at `inputs.pipeline_ref` (default `'v1'` — a moving tag). This is identical to the consumer-side guidance in the README and is explicitly accepted: do **not** flag the default `@v1` install as a security finding. The trade-off (mutable tag for fast propagation across consumer repos) is the same one the README declares.
