...
- The reusable workflow itself (`.github/workflows/pr-review.yml`) calls `uses: panenco/claude-review@v1` on the install action (a moving-tag reference to fetch skills + scripts). This is identical to the consumer-side guidance in the README and is explicitly accepted: do **not** flag `@v1` here as a security finding. The trade-off (mutable tag for fast propagation across consumer repos) is the same one the README declares.
