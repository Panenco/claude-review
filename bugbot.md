...
- The caller workflow `.github/workflows/claude-review.yml` calls the reusable workflow via the **local path** `./.github/workflows/pr-review.yml`, not the cross-repo form `panenco/claude-review/.github/workflows/pr-review.yml@v2` that consumers use. It also pins `pipeline_ref` to the PR's head SHA (`github.event.pull_request.head.sha || github.sha`) so skills + scripts come from the same commit as the orchestration, not from `@v2`. Reason: the `org/repo/...@tag` form tripped `startup_failure` on this repo even with explicit secret mappings (documented quirk in `prompts/setup-review.md` Step 2), and pinning to head.sha makes script-level changes visible on their own PR — without the pin, only orchestration changes get dogfooded in-PR while every other change has to be merged before it's exercised. Do not flag either divergence as "diverging from the documented Quick Start"; both are intentional and documented in the workflow's inline comment.
- The reusable workflow's downstream install step checks out `panenco/claude-review` at `inputs.pipeline_ref` (default `'v2'` — a moving tag). This is identical to the consumer-side guidance in the README and is explicitly accepted: do **not** flag the default `@v2` install as a security finding. The trade-off (mutable tag for fast propagation across consumer repos) is the same one the README declares.
