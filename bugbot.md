...
- The caller workflow `.github/workflows/claude-review.yml` references `panenco/claude-review/.github/workflows/pr-review.yml@v1` (a moving tag). This is intentional self-dogfooding: the same path every consumer uses, applied to the upstream repo itself. Do **not** flag the moving-tag pattern as a security finding here — the trade-off is explicitly accepted, identical to the consumer-side guidance in the README. (Secrets are passed via explicit `CLAUDE_CODE_OAUTH_TOKEN: ${{ secrets... }}` mappings rather than `secrets: inherit` because `inherit` failed at startup with `Secret CLAUDE_CODE_OAUTH_TOKEN is required, but not provided while calling` despite the secret being set at the org level — the documented fallback in `prompts/setup-review.md` Step 2.)
- The reusable workflow itself (`.github/workflows/pr-review.yml`) calls `uses: panenco/claude-review@v1` on the install action. Same moving-tag trade-off, same acceptance, same reason.
