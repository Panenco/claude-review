...
- "Install functional-tester subagent" copies the pipeline's static `agents/review-functional-tester.md` to `~/.claude/agents/` on the runner, templated with `inputs.model_functional` (it pins the tester's model and points at its skill — don't commit such a file to your repo)
