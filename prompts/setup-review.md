...
### The `native` pass — nothing to wire, but it is live

`/review native` runs Anthropic's official `code-review` plugin in-session as a second opinion. It was deleted in v4 and is **back as of `docs/adr/0005-native-pass-returns-pinned.md`**, now installed from a marketplace the reusable workflow vendors at a pinned commit SHA. It is opt-in (`/review native` or `/review all`), advisory, and it needs **no caller-side wiring at all** — do not add `plugin_marketplaces` or `plugins` to a caller workflow; those are the reusable workflow's business.

Two optional inputs steer it, and both are live again after a spell as deprecated no-ops: `native_review_scope` (free text that NARROWS the pass to some paths, e.g. `only review changes under apps/api/`) and `model_standard` (the model that runs it). Pass them only if the repo wants them; the defaults are fine. `native_review` and `native_review_runner` never existed in v3 and still do not; passing either fails the run with `startup_failure`.
...
- Dev env setup starts your services (look for `API ready at ...` in logs — not just `API=false`). On a review that starts no dev env (a code-only run, or a repo with no `dev-start.sh`) the orchestrator prints `DEV_ENV_WAIT=0` and moves straight on — it does not sit out `dev_env_timeout_seconds` waiting for a bring-up that was never launched. When one *is* launched, that timeout is clamped to 540s: the wait lives inside a single Bash tool call and that tool kills its own call at 600s, losing the call's whole output. Passing a bigger `dev_env_timeout_seconds` buys nothing
- "Install review subagents" copies the pipeline's static `agents/review-scan.md`, `agents/review-verify.md` and `agents/review-functional-tester.md` to `~/.claude/agents/` on the runner, templated with `inputs.model_high` / `inputs.model_functional` (each pins its model and its reasoning effort and points at its skill — don't commit such files to your repo). `agents/review-native.md` joins them on a `/review native` run, and only when the pinned plugin marketplace resolved
- The orchestrator session itself runs sonnet-5 (`model_orchestrator`), not the reviewing model: it dispatches subagents and copies one file, it never reviews the diff, and its model never reaches a subagent — each one carries its own `model:` in the frontmatter above. Leave it alone unless you have a reason.
