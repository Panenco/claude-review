...
# there is no token to turn it off. The browser tester (`functional`), the
# second-opinion plugin pass (`native`) and the guard's size-ceiling override
# (`deep`) are opt-in.
#
...
#   run_native=true|false            second-opinion pass: Anthropic's `code-review`
#                                    plugin, run in-session from a SHA-pinned
#                                    vendored marketplace (ADR 0005).
...
#   message=<single-line reply>      help/unknown
#
...
emit() {
  # $1 action, $2 run_functional, $3 force_deep, $4 passes, $5 message, $6 run_native
  printf 'action=%s\nrun_functional=%s\nrun_native=%s\nforce_deep=%s\npasses=%s\nmessage=%s\n' \
    "$1" "$2" "${6:-false}" "$3" "$4" "$5"
}
help_msg() {
  printf '%s' "Comment \`$TRIGGER code\` (code review) or \`$TRIGGER functional\` (+ browser test), or \`$TRIGGER all\` for both — combine them in ONE comment; separate comments queue up and post separate reviews. Add \`$TRIGGER native\` for a second opinion from Anthropic's official \`code-review\` plugin, run in-session. Add \`deep\` (e.g. \`$TRIGGER code deep\`) to review a PR that is over the size ceiling; it applies to that run only, while the \`deep-review\` label does the same on every push. Otherwise the reviewer scales its own depth."
}
...
if [ "$EVENT" = "workflow_dispatch" ] && [ -z "${FIRST_LINE//[[:space:]]/}" ]; then
  emit "run" "false" "false" "code" "" "false"
  exit 0
fi
...
case "$FIRST_LINE" in
  ...
  *)
    emit "ignore" "false" "false" "" "Not a $TRIGGER command." "false"
    exit 0
    ;;
esac
...
for raw in $ARGS; do
  ...
  case "$tok" in
    ...
    # Anthropic's official `code-review` plugin, in-session second opinion.
    native|anthropic|plugin)       run_native=true; add_pass code; add_pass native ;;
    all|everything)                run_functional=true; run_native=true
    ...
    help|\?)
      emit "help" "false" "false" "" "$(help_msg)" "false"
      exit 0
      ;;
    *)
      emit "unknown" "false" "false" "" "Unknown option \`$tok\`. $(help_msg)" "false"
      exit 0
      ;;
  esac
done
...
emit "run" "$run_functional" "$force_deep" "$passes" "" "$run_native"
