...
- `requires_human_review` is your honest judgment that a human should look before this merges — set it when you would otherwise APPROVE but cannot, in good conscience, **vouch** for the change at high confidence. This is a **deduction, not a category list**: a complex or business-critical component is fully APPROVE-able when the spec is clear, you verified the change is correct and safe, and the quality is high. Set it only when a **consequential** change carries genuine residual uncertainty you could not resolve — the spec for a critical path is missing/ambiguous so you can't confirm intent; the behaviour is high-blast-radius and you couldn't verify it (no functional evidence, couldn't fully trace it); or it's subtle enough that you're genuinely unsure a defect wouldn't slip past you. **Keep this rare**: if you found nothing wrong AND you can confidently stand behind the change, APPROVE — never use human-review as a hedge for ordinary uncertainty or to dodge the call. A concrete defect (including missing auth) is a **finding**, not a human-review punt. `requires_human_review_reason` must name the specific residual doubt.
