...
Then emit it as a normal finding meeting the full bar (`path`, in-hunk `line`, `title`, `failure_scenario` = the observed behaviour, `fix`, `severity`). If you cannot tie it to a changed line, **discard it**, silently, like the rest of that file. It is not a `human_review` note either: that channel is orientation on a block of changed code, not a parking space for an observation you could not place.
...
- **APPROVE** — requires ALL of: zero surviving findings; a real, non-empty `approve_argument` from scan; `sensitive_paths_touched` false; `review_effort` ≤ 3. **On a `DOCS_ONLY` run — `DOCS_ONLY` is in your env — add one more: zero surviving notes.**
  - **Surviving notes never block APPROVE on a code diff, and zero notes is a reason to give it.** A note is a reading aid, not a risk signal. "No notes" used to mean "nobody needs to read this diff"; it now means no block needed orienting, which on a CRUD endpoint, a form or a straightforward business rule is the normal and correct result. **A clean simple PR should APPROVE — expect that on a third to a half of code PRs** — and reaching for a COMMENT because the review looks thin is padding by another route.
