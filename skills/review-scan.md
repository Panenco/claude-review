...
**Then read every default an infra or config diff introduces as a decision, with the stack that never set the value as the input.** `?? true` on a new flag, a timeout raised on a shared service, a memory size, an IAM grant, a subscription's ack deadline. The scenario is the stack where nobody set it: a flag that defaults to on where it is unset turns the feature on for a stack that never opted in, and a request timeout raised for one slow job is raised for every request the service takes. Ordinary finding, ordinary bar — the wrong output is what that stack now does. **A knob raised on a shared service is scored on the other callers, not on the job that asked for it**: the scenario for a request timeout doubled to 600s is the hung request from any other route that now holds a Cloud Run instance twice as long, and "the job needs it" is the fix's problem, not the finding's.

**Then open what the new code was copied from, and list what did not come across.** The passes above hunt for something wrong *in* the diff. This one hunts for something *absent* from it, which no amount of tracing the new lines can surface: every line you are reading is correct, and the defect is the line that is not there. It applies whenever the diff adds a thing that already has an established counterpart in this repo — a second tab under the same shell, another endpoint on the same controller, a second screen behind the same feature flag, another consumer of a shared hook. Find the counterpart, read what it does *beyond* its happy path (the guard, the wrapper, the cleanup, the entry in some inventory, the dirty-state check), and tick each one against the new code. A gap is an ordinary finding at the ordinary bar, and the counterpart hands you the failure scenario, because it is the failure somebody already hit and fixed there.

**Naming a sibling is not the same as reading it, and this is where the pass fails.** A review that approved a new page said it "mirrors the established sibling exactly" — on the strength of a matching *filename*. It had not opened the other half of the pair, which wraps its panel in a component the new page omitted, nor a second sibling carrying an unsaved-changes guard the new page also lacked, so switching tabs mid-edit dropped the edit silently. Three findings, all sitting in code the review had already named. **"It matches the existing pattern" is a finding-sized claim.** Earn it by listing what the older one does, or do not make it.

**An infra module, a job definition or a workflow file has a counterpart too: its sibling in the same directory.** The new Cloud Run job beside the existing ones, the new deploy step beside the last one. Read what the sibling wires into its service beyond the image and the command — the error-reporting DSN, the env block, the secrets, the scaling knobs — and tick each against the new one. A 40-minute job that ships without the Sentry env every sibling carries fails silently for 40 minutes; that is the failure scenario, and the sibling supplied it.

**A file this diff moved, copied or re-implemented from is the strongest counterpart there is, and the easiest to skip.** `git diff --diff-filter=DR --name-status origin/<base>...HEAD` lists the moves; a *copy* leaves the source in place and appears nowhere in that list, so read the PR body and the new file's own header for where it came from (measured: "`apps/doc-templates/src/lib/agent/` is now `apps/api/.../lib/agent/`", with the POC left intact). `git show origin/<base>:<old path>` is the text to read. Walk the old file and list what did not come across: every `describe`/`it` block, every guard, every export. A test the move dropped is a finding when the code it guarded still ships, and the old test hands you the scenario ready-made — its input is the input, and the behaviour it asserted is what nothing checks now. **That the new code still passes the behaviour by inspection does not close it**: the finding is that the guard has no test any more, and the scenario is the old test's input against the next edit to that code. When the spec says the tests move with their assertions unchanged, quote that line as `evidence` and it is a spec finding besides. Measured: a move carried an agent into the API and dropped three path-containment test blocks while the code they guarded still read and wrote the filesystem, and the review approved the move on the strength of "the moved tests pass".

**The bound: only what this code's own job also needs.** A guard against a state this screen can also reach, a registration this file also requires, a cleanup this path also leaks without. A capability the counterpart has because *its* feature asked for it is not a gap, and "be more like the neighbour" is not a finding. **Observability, secrets and platform env are never the feature's: the error-reporting DSN, the log sink, the auth secret, the env every sibling service or job is wired with.** Their absence on the new one is always this pass's finding, whatever the new one's feature is — a job that runs for forty minutes and reports nothing when it dies is the scenario, and you do not need to guess it.
...
**A spec's negative constraints are criteria.** "There is no callback controller", "no `@SystemGuarded()` route", "after this step nothing writes to disk", "runs once" bind exactly like the positive ones, and they are the ones a diff breaks most quietly, because nothing in the new code looks wrong: the defect is that the document says it does not exist. For each such sentence in the governing source, `Grep` the diff for the thing it rules out. Code that adds it is an ordinary finding, and the scenario is supplied: **the next slice's implementer reads that sentence and builds against a constraint the code already broke** — `severity: minor` at least, `evidence` quoting the sentence and the line that breaks it. When the PR body argues the reason and the reason holds, the code is not what is wrong, the document is: keep the finding, anchor it on the code, and make `fix` "correct the plan in this PR", naming the sentence. Having grepped for it and found it is not the end of the pass; filing it is. A reviewer of the next slice inherits whatever the plan says, so a plan the code already contradicts is a live defect, not a stale comment. A claim about the state *after* this step is checked the same way: "nothing writes to files any more" means tracing whether anything at HEAD still reaches the old path — a branch that is only ever taken because the column that would skip it is never set, is the plan not delivered.
...
**A consequence a named person hits is a finding, not a note.** When the thing the reader cannot see is that someone — a dev on a machine without the new binary, an operator on the next deploy, a user of the sibling screen — now meets a failure they did not meet before, you are holding a finding: that person is the input, the failure is the wrong output, and it goes through the finding bar, not this channel. A note that stops one sentence short of the harm is the most expensive miss this pipeline makes. Measured on a real PR: the note said "adding `pdftohtml` to the binary map makes it a required ingest binary, the boot check enforces it"; the human reviewer said "every dev machine without poppler-utils now gets an API that dies at boot, I hit exactly that", and only one of those changed the PR. Before you write a note, finish the sentence — *so who hits what* — and if it ends on a person and a failure, file it.
...
- **A sentence that names who now hits what.** "An API on a machine with an older poppler now refuses to boot", "a local POST now calls the live Google API", "the next slice's reviewer inherits a plan the code already breaks". That is a finding with its scenario already written, and the note channel is the wrong door for it: file it as a `minor` (or higher) finding at that block instead. Verify is told to relabel one that arrives here anyway, so writing it as a note buys nothing.
