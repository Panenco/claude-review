...
**Naming a sibling is not the same as reading it, and this is where the pass fails.** Measured on qiv#1510: the review stated the new page "mirrors the established sibling (documents-tabs.ts / documents-content.tsx) exactly" and approved. It had compared the file with the matching *name*. The other half of that pair wraps its panel in `<Prototype>` and the new page did not; a second sibling, `configuration-content.tsx`, carried an `isDirty`/`pendingSubtab` guard the new page also lacked, so switching tabs mid-edit dropped the edit silently — confirmed by the author as real data loss. Three findings, all of this shape, all sitting in code the review had already named. **"It matches the existing pattern" is a finding-sized claim.** Earn it by listing what the older one does, or do not make it.
...
**A premise you did not read is not evidence.** Where the failure scenario turns on how something *outside the diff* behaves — a marketplace action, the CI runner model, a library default, another repo's config — you must have read that thing in this checkout and quoted it in `evidence`. It is not on disk, so you cannot check it, so there is no finding. Both rebuttals of the form "your premise is inverted" measured on 31 Aug were this: one review asserted that `actions/create-github-app-token` narrows a token when `owner:` is set, where the action's own `action.yml` says unset already means this repository only and setting it *widens*; the other assumed shared CI runners on a repo whose runners are ephemeral per-job pods. In both the author went and read the source the review had not. Your sense of how a tool usually works is the weakest thing a finding can rest on, and the fastest for an author to refute.
