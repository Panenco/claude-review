...
**Second opinion: the native `code-review` pass (`native_review`).** Every review runs **two** reviewers in parallel. The first is this pipeline (the `In-Depth Review` job — orchestrator, two judges, functional tester). The second is the `Native Review` job: the official Claude `code-review` plugin, run exactly as Anthropic ships it, on the same PR head. It is on by default and needs no caller change — its permissions (`contents: read`, `pull-requests: write`, `issues: write`) are a strict subset of what the caller already grants. It honours the same review plan as the main review, so the `gate_skip_label` (default `skip-review`) and the oversized ceilings silence **both** reviewers, not just one.
