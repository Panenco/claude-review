...
**Second opinion: the native `code-review` pass (`native_review`).** Every `full` review runs **two** reviewers: this pipeline's own panel (orchestrator, two debating judges, functional tester) and the official Claude `code-review` plugin, run exactly as Anthropic ships it. The second one is not a separate job — it is the `review-native` subagent, dispatched by the orchestrator **inside the existing review session**, in the same response as the context builder. It therefore costs **no extra runner, no second checkout, no second Claude CLI install and no second OAuth probe**, and it adds no wall-clock: it runs concurrently with the context build, the judges and the functional tester. It is on by default and needs no caller change.
